...
int main(int argc , const char* argv[])
{
  ...
  fgets(line,512,fp); //get edges num
  size = atoi(line);//save it
  t= createTable(size,1,size);
  if(!t)
    printf("Table Not Created\n");
  createDB(t,fp);//add data to table
  temp = search(t, src);
  if(temp==NULL)
  {
  ...
  temp->weight=0;
  chk = findShortestPath(t,src);//running bellman ford
  if(!chk)//if negative circle is found
  {
    printf("\nERROR: Negative Circle Found\n");
    set_to_infinity(t);
  }
  toString(t,src);//print result
  freeTable(t);//keep memory clean
  exit(EXIT_SUCCESS);
}
